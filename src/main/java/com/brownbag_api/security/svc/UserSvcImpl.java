		...
		if (strRoles == null) {
			...
			strRoles.forEach(role -> {
				...
				case "broker":
					...
					break;
				case "trader":
					Role modRole = roleRepo.findByName(ERole.ROLE_TRADER).orElseThrow(
							() -> new RuntimeException("Error: Role  " + ERole.ROLE_TRADER + " is not found."));
					roles.add(modRole);

					break;
				default:
					Role userRole = roleRepo.findByName(ERole.ROLE_TRADER).orElseThrow(
							() -> new RuntimeException("Error: Role " + ERole.ROLE_TRADER + " is not found."));
					roles.add(userRole);
				}
			});
